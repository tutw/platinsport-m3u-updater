...
def evento_acabado(hora_evento):
    ...
    hora_evento_completa = datetime.combine(hora_actual.date(), hora_evento)
    
    # Comparar si el evento terminó hace más de 3 horas
    if hora_evento_completa < hora_actual - timedelta(hours=3):
        return True
    return False
