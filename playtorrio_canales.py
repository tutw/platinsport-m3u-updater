...
def generate_m3u(channels, logo_map, output_path='playtorrio_canales.m3u'):
    ...
    for ch in channels:
        ...
        # Construir línea EXTINF (formato solicitado)
        # - Sin tvg-id
        # - Sin tvg-name
        # - group-title = código de país en MAYÚSCULAS (p.ej. DE)
        # - Nombre mostrado: sólo ch['name'] (sin [País])
        group = (ch.get('country_code') or '').upper() or 'XX'
