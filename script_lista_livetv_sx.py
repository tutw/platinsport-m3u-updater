...
def parse_event_urls_and_details(html_content: str) -> List[Event]:
    ...
    Se enfoca en los eventos que siguen a un encabezado de fecha y ignora la sección "Top Events LIVE".
    """
    ...
    # Ajustar a la tabla correcta, si hay varias con width=230, podríamos necesitar un padre específico
    # Buscamos la tabla con id 'allmatches' si existe, o el patrón anterior
    main_table = soup.find('table', id='allmatches')
    if not main_table:
        main_table = soup.find('table', width='230', cellspacing='0')

    if not main_table:
        logging.warning("No se encontró la tabla principal de eventos (id='allmatches' o width='230').")
        return found_events

    # Iterar sobre las filas (<tr>) de la tabla principal
    for tr in main_table.find_all('tr', recursive=False): # recursive=False para solo hijos directos
        ...
        if date_span:
            ...
            if "Top Events LIVE" in date_text:
                parsing_events_after_date = False # Estamos en la sección de top events, ignoramos por ahora
                logging.debug(f"Ignorando sección: {date_text}")
                continue # Saltamos al siguiente tr

            # Si es un encabezado de fecha real, actualizamos la fecha y activamos el flag
                ...
                if "Hoy (" in date_text:
                    ...
                    if date_match:
                        ...
                        try:
                            ...
                            if month:
                                # Heurística para el año: si el mes ya pasó en el año actual, asumimos el próximo año.
                                # O si es el mismo mes pero el día ya pasó, también asumimos el próximo año.
                                # Esto es para fechas como "26 de mayo" si estamos en junio.
        ...
        if parsing_events_after_date:
                    ...
                    if link and 'href' in link.attrs:
                        ...
                        if match:
                            ...
                            if img_tag and img_tag['alt']:
                                ...
                                sport_from_img = re.sub(r'^(Tenis|Fútbol|Críquet|Automovilismo|Baloncesto|Hockey)\.\s*', '', sport_from_img, flags=re.IGNORECASE).strip()
                                sport_from_img = re.sub(r'^(ATP|WTA|NHL|NBA|Liga MX)\.\s*', '', sport_from_img, flags=re.IGNORECASE).strip()
            ...
            logging.debug("Saltando TR, no estamos en una sección de eventos post-fecha.")
